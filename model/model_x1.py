...
import os
from model.HDModelBlock import SpaAngRegResB_4D, HDRegression_4d, SAResB3D
from model.HDModelBlock import SA_4Dattention2, Depth_5Dattention, downsample_lf
from model.HDSRBlock import SuperWrap2, concatenate_blocks_3d_amend
...
class Net(nn.Module):
    def __init__(self, cfg):
        ...
        self.super_regression = HDRegression_4d(mindisp, maxdisp)
    ...
    def forward(self, x):
        ...
        low_cost = self.aggregation(cost)
        cost = rearrange(low_cost, 'b c (d s) h w -> b c d s h w', s=(self.Scale_target)**2).contiguous()
        low_disp = self.super_regression(cost.squeeze(dim=1))  # (b,c,s,h,w)
        
        # 将视差维度信息变成超分信息 (b,c,s,h,w) -> (b,c,H,W)
        super_disp = concatenate_blocks_3d_amend(low_disp)

        ## 总的下采样
        super_disp_down = downsample_lf(super_disp, 1/self.Scale_target)
