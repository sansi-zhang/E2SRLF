...
class TrainSetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        """ Read inputs """
        for i in range(81):
            temp = imageio.v2.imread(self.trainset_dir + scene_name + '/input_Cam0%.2d.png' % i)
            lf[i // 9, i - 9 * (i // 9), :, :, :] = temp
            
        # center_image_index = (4, 4)
        # lf = lf[center_image_index[0]-3:center_image_index[0]+4, center_image_index[1]-3:center_image_index[1]+4, :, :, :]

        
        
        dispGT = np.zeros(shape=(512, 512, 2), dtype=float)
        dispGT[:, :, 0] = np.float32(read_pfm(self.trainset_dir + scene_name + '/gt_disp_lowres.pfm'))
        mask_rgb = imageio.v2.imread(self.trainset_dir + scene_name + '/valid_mask.png')
        ...
        while (sum_diff < 0.01):
            ...
            # txtfile = open('scene_id.txt', 'a')
            # txtfile.write('scene_id={}'.format(scene_id))
            # txtfile.close()
        ...
        data_mini = downsample_lf(lf_crop, scale_factor)
        data = rearrange(data_mini, 'a1 a2 h w -> (a1 h) (a2 w)', a1=self.angRes, a2=self.angRes)
...
def refocus_augmentation(lf, dispGT):
    ...
    if refocus_rand == 0:
        ...
        # min_d = int(np.nanmin(dispGT[:, :, 0]))
        # max_d = int(np.nanmax(dispGT[:, :, 0]))
        min_d = int(np.min(np.nan_to_num(dispGT[:, :, 0], nan=0)))
        max_d = int(np.max(np.nan_to_num(dispGT[:, :, 0], nan=0)))
